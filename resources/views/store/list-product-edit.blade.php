    ...
    {!! Form::model($product, ['method' => 'POST', 'route' => ['save-products', $product->id], 'class' => "form-edit-product " . ($product->related ? 'product-variation' : '') . " " . ($product->status == '0' ? 'product-disabled' : ''), 'files' => true, 'data-related' => $product->related ?? '']) !!}
        ...
        <div class="row">
            ...
            <div class="col-xs-6 options">
                ...
                <button type="button" class="free-freight {{ $product->free_freight ? 'free-freight-selected' : '' }}" title="{{ $product->free_freight ? 'Desabilitar frete grátis' : 'Habilitar frete grátis' }}" data-url="{{ route('product-free-freight') }}">frete grátis</button>
            </div>
        </div>
