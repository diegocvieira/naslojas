@foreach ($products as $product)
    <div class="product {{ $product->related ? 'product-variation' : '' }} {{ ($product->status == 0 || $product->status == 2) ? 'disabled' : '' }}" data-storestatus="{{ $product->store->status == 1 ? true : false }}" data-related="{{ $product->related ?? '' }}" data-slug="{{ $product->slug }}">
