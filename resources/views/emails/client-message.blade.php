            ...
            <table id="Tabela_01" style="margin-bottom: 30px;" width="600" height="auto" border="0" cellpadding="0" cellspacing="0" align="center">
                ...
                <tr>
                    <td style="box-shadow: 5px 8.66px 9px 1px rgba(100, 100, 100, 0.118); background-color: #fff; padding: 10px 50px 10px 50px;">
                        <span style="display: block; font-size: 14.5px; color: rgb(100, 100, 100);">A loja {{ $message->product->store->name }} respondeu a sua mensagem sobre o seguinte produto:</span>
                    </td>
                </tr>

                <tr>
                    <td style="box-shadow: 5px 8.66px 9px 1px rgba(100, 100, 100, 0.118); background-color: #fff; padding: 40px 50px 0 50px;">
                        <img src="{{ asset('uploads/' . $message->product->store_id . '/products/' . $message->product->images->first()->image) }}" style="float: left; margin-right: 10px; width: 248px; height: 248px; object-fit: cover;" />
                        ...
                        <span style="margin: 10px 0; display: inline-block; font-weight: 700; font-size: 16.6px; color: rgb(50, 50, 50)"><span style="font-size: 12.5px;">R$</span> {{ number_format($message->product->price, 2, ',', '.') }}</span>

                        @if ($message->product->old_price)
                            <span style="display: inline-block; margin-left: 10px; font-size: 14.5px; color: rgb(112, 202, 124);">{{ _discount($message->product->price, $message->product->old_price) }}% OFF</span>
                        @endif

                        @if ($message->product->installment && $message->product->installment_price)
                            <span style="font-size: 12.5px; display: block; color: rgb(50, 50, 50);">
                        ...
                        @if ($message->product->reserve && $message->product->reserve_discount)
                            <span style="color: #000; display: block; position: relative; left: -10px; font-size: 14.5px; margin-top: 10px;" title="Preço com desconto reservado pelo naslojas"><span style="font-size: 16.6px; font-weight: 700; background-color: rgb(255, 215, 223); padding: 5px 10px; border-radius: 25px; margin-right: 5px;">R$ {{ number_format(_reservePrice($message->product->price, $message->product->reserve_discount), 2, ',', '.') }}</span> na reserva pelo <i>naslojas</i></span>
                        ...
                        <span style="margin: 10px 0; display: block; font-size: 14.5px; color: rgb(150, 150, 150); line-height: 1.286;">{{ $message->product->title }}</span>
                ...
                <tr>
                    <td style="box-shadow: 5px 8.66px 9px 1px rgba(100, 100, 100, 0.118); background-color: #fff; padding: 40px 50px 10px 50px;">
                        <span style="display: block; text-transform: uppercase; font-weight: 700; font-size: 14.5px; color: rgb(50, 50, 50);">{{ $message->product->store->name }}</span>
                    </td>
                </tr>

                <tr>
                    <td style="box-shadow: 5px 8.66px 9px 1px rgba(100, 100, 100, 0.118); background-color: #fff; padding: 0 50px;">
                        <span style="display: block; font-size: 14.5; color: rgb(150, 150, 150); line-height: 1.429;">{{ $message->response }}</span>
                    </td>
                </tr>
                ...
                <tr>
                    <td style="box-shadow: 5px 8.66px 9px 1px rgba(100, 100, 100, 0.118); border-radius: 0 0 5px 5px; background-color: #fff; padding: 40px 50px 50px 50px;">
                        <a href="{{ route('show-product', $message->product->slug) }}" style="display: inline-block; border-radius: 25px; font-weight: 700; font-size: 14.5; margin-top: 10px; background-color: rgb(255, 23, 68); color: #fff; padding: 13px 25px;">ENVIAR OUTRA MENSAGEM</a>
                    </td>
                </tr>
